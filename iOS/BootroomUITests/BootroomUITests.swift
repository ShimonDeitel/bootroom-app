...
final class BootroomUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<(Self.freeLimitForUITest + 1) {
            ...
            if nameField.waitForExistence(timeout: 2) {
                ...
                app.buttons["entrySaveButton"].tap()
            } else if app.staticTexts["Bootroom Pro"].waitForExistence(timeout: 2) {
                break
            }
        }
        XCTAssertTrue(app.staticTexts["Bootroom Pro"].waitForExistence(timeout: 3))
    }
